...
return (
<motion.div 
className='flex flex-col justify-between bg-zinc-950 text-gray-400 h-full overfolw-y-scroll min-w-210 hide-scrollbar'>
    ...
    <div className="flex text-gray-400 flex-col gap-5">
        ...
        categories.slice(0 , categories.length - 1).map((categ) => {
            ...
            >
                {/* <img 
                src={categ.image} alt={categ.name} className='rounded-full w-9 h-9 '
                />
                {categ.name} */}
                <MotionLink 
                    heading = {categ.name}
